...
function WeightTooltip({ active, payload, label }: { active?: boolean; payload?: { value: number }[]; label?: string }) {
  ...
  return (
    <div className="rounded-xl bg-popover px-3 py-2 text-caption ring-1 ring-border">
      ...
      <p className="mt-0.5 tabular font-medium text-foreground">{payload[0].value.toFixed(1)} lbs</p>
    </div>
  );
}
    ...
    <Card>
      <div className="flex items-start justify-between">
        <div>
          ...
          {latest ? (
            <>
              <p className="mt-1 tabular text-title font-bold text-foreground">{formatLbs(Number(latest.weight_kg), 1)} lbs</p>
